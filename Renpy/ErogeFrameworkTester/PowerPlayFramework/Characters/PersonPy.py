...
class Minimal_Character(fundamentals.Buildable_From_Json):
    ...
    def build_from_json(self, id, json):
        ...
        if "names" in json.keys():
            names = Character_Names().build_from_json(json["names"])
        else:
            # TODO: Generate random value instead of rasing and error.
            renpy.error("ERROR: Minimal_Character.build_from_json(): Character does not have a 'names' field.")
        if "gender" in json.keys():
            gender = json["gender"]
        else:
            # TODO: Generate random value instead of rasing and error.
            renpy.error("ERROR: Minimal_Character.build_from_json(): Character does not have a 'gender' field.")
        if "age" in json.keys():
            age = int(json["age"])
        else:
            # TODO: Generate random value instead of rasing and error.
